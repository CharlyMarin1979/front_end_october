...
export class DetallePeliculaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe(params =>{
      this.peliculasService.ontenerPorId(params.id).subscribe(pelicula => {
        console.log(pelicula);
        this.pelicula = pelicula;
        this.fechaLanzamiento = new Date(this.pelicula.fechaLanzamiento);
        this.trailerURL = this.generarURLYTEmbeded(this.pelicula.trailer);
  ...
  generarURLYTEmbeded(url: any): SafeResourceUrl{
    if(!url){
      return '';
    }

    var video_id = url.split('v=')[1];
    var posicionApersand = video_id.indexOf('&');
    if (posicionApersand !== -1){
      video_id = video_id.substring(0, posicionApersand);
    }
